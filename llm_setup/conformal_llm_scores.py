...
token_limit = 2048  # Maximum size of tokens used in forward pass.
...
save_dir = '../models/Llama-3.2-1B-Instruct'  # Model Directory

tokenizer = AutoTokenizer.from_pretrained(save_dir)#, low_cpu_mem_usage=True)
model = AutoModelForCausalLM.from_pretrained(save_dir)#, low_cpu_mem_usage=True)
tokenizer.pad_token = tokenizer.eos_token
model.config.pad_token_id = model.config.eos_token_id
model.cuda()#half().cuda()

# Get prediction for subjects with MMLU based prompts

acc_dicts = {}

for subject_name in tqdm(task_list, desc="Processing Subjects"):
    task_data = load_dataset('lukaemon/mmlu', subject_name)
    ...
    for j, question_num in enumerate(tqdm(prompt_question_ids_dict[subject_name], desc=f"Processing prompts for {subject_name}")):
        preds = []
        targets = []
        ...
        for i, (question, answer) in enumerate(tqdm(zip(questions, answers), 
                                                   desc=f"Processing questions for prompt {j}", 
                                                   total=len(questions))):
